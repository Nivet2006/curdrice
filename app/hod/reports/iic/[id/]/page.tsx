...
import { HODIICAuditWrapper } from '@/components/hod/HODIICAuditWrapper'
import { ArrowLeft, Building, ShieldCheck, FileText, ExternalLink } from 'lucide-react'
import Link from 'next/link'

export default async function HODIICReportAuditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-12 pb-24 px-4 sm:px-6">
      <div className="flex items-center justify-between">
        <Link href="/hod/dashboard" className="flex items-center gap-2 text-zinc-400 hover:text-black dark:hover:text-white font-mono text-[10px] uppercase font-black tracking-widest transition-all">
          <ArrowLeft size={14} />
          HOD Dashboard
        </Link>
        <div className="flex items-center gap-3">
          <Building size={16} className="text-black dark:text-white" />
          <span className="font-mono text-xs uppercase text-white bg-black dark:bg-white dark:text-black px-4 py-1.5 rounded-full border-2 border-black dark:border-white font-black">HOD Compliance Review</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 lg:sticky lg:top-24 h-fit">
          <HODIICAuditWrapper reportId={report.id} reportStatus={report.status} rejectionFeedback={report.rejection_feedback} />
        </div>
      </div>
    </div>
  )
