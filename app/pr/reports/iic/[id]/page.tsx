...
import { PRIICAuditWrapper } from '@/components/iic/PRIICAuditWrapper'
import { ArrowLeft, Megaphone, Calendar, DollarSign, Users, Award, ExternalLink, CalendarDays, CheckCircle, ShieldAlert, AlertTriangle } from 'lucide-react'
    ...
    <div className="max-w-[1200px] mx-auto space-y-12 pb-24 px-4 sm:px-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-xs uppercase text-white bg-black dark:bg-white dark:text-black px-4 py-1.5 rounded-full border-2 border-black dark:border-white font-black">Official IIC Report Audit</span>
        </div>
      </div>
      ...
      {/* Show Faculty rejection annotations if rejected back to PR */}
      {existingAnnotations.length > 0 && report.status === 'rejected_faculty' && (
        <div className="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 p-6 rounded-2xl space-y-4">
          <div className="flex items-center gap-2">
            <AlertTriangle size={16} className="text-amber-500" />
            <h3 className="text-sm font-black uppercase text-amber-700 dark:text-amber-400">Faculty Verification Feedback</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {existingAnnotations.map((a, i) => (
              <div key={i} className="bg-white dark:bg-zinc-900 border border-amber-200 dark:border-amber-500/20 p-4 rounded-xl">
                <p className="text-[9px] font-mono text-amber-500 uppercase tracking-widest mb-1">{a.section.replace('_', ' ')}</p>
                <p className="text-sm text-zinc-700 dark:text-zinc-300 font-medium">{a.comment}</p>
              </div>
            ))}
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#f5f5f5] dark:bg-zinc-800/50 text-[#555555] dark:text-[#a0a0a0] text-[10px] uppercase tracking-wider font-mono">
                      ...
                      <th className="p-3 font-semibold text-right">Checked In At</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#e0e0e0] dark:divide-zinc-800 font-sans text-xs">
                    {totalRegistered === 0 ? (
                      <tr>
                        <td colSpan={5} className="p-8 text-center text-[#999999] dark:text-zinc-500 font-mono">
                      ...
                      registrations.map((reg: any) => (
                        <tr key={reg.id} className="hover:bg-[#fcfcfc] dark:hover:bg-zinc-900/50 transition-colors">
                          ...
                          <td className="p-3 font-mono text-[10px] text-[#999999] text-right whitespace-nowrap">
