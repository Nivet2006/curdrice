          ...
          {questions.length === 0 ? (
            ...
            <div className="space-y-4">
              {questions.map((q) => (
                <div 
                  key={q.id}
                  className="group bg-white border border-zinc-200 shadow-sm rounded-2xl p-6 relative"
                >
                  <QuestionItemContent q={q} />
                </div>
              ))}
            </div>
          )}
