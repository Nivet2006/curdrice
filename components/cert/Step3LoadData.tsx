...
export function Step3LoadData({
  ...
  const [activeTab, setActiveTab] = React.useState<'file' | 'manual'>('file');
  ...
  // Tab change handler
  const handleTabChange = (tab: 'file' | 'manual') => {
    setActiveTab(tab);
    setError(null);
    if (tab === 'manual') {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fields.forEach(f => {
        const matchingCol = data.headers.find(h => h.toLowerCase() === f.label.toLowerCase());
        if (matchingCol) {
          onUpdateFieldMapping(f.id, matchingCol);
        } else {
          onUpdateFieldMapping(f.id, data.headers[0] || null);
        }
      });
    } catch (err) {
      console.error(err);
