      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-4 flex-1">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-[#1F3A8A] dark:text-blue-400">
              <FileText size={20} />
            </div>
            <div>
          ...
          <div className="space-y-2 max-w-md">
            <div className="flex items-center justify-between text-xs font-medium text-zinc-600 dark:text-zinc-400">
              ...
              <span className={isComplete ? "text-green-600 dark:text-green-400 font-bold" : ""}>
                {status?.submitted || 0} / {status?.total || 0} Students
              </span>
            </div>
            <div className="h-2 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
              <div 
                className={`h-full transition-all duration-1000 ${isComplete ? 'bg-green-600 dark:bg-green-500' : 'bg-amber-500'}`}
                style={{ width: `${progressPercent}%` }}
              ></div>
            </div>
            {isComplete ? (
              <p className="text-xs text-green-600 flex items-center gap-1.5 mt-2 font-medium">
                <CheckCircle2 size={14} /> All student feedback collected
              </p>
            ) : (
        ...
        <div className="flex flex-col gap-3 min-w-[200px]">
          {isComplete || reportData ? (
            <Link 
              ...
              className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#1F3A8A] to-[#2563EB] hover:from-[#1e3a8a] hover:to-[#1d4ed8] text-white font-medium shadow-md shadow-blue-500/20 transition-all active:scale-[0.98]"
            >
              <FileText size={16} />
          ...
          {reportData?.pdf_url && (
            <a 
              ...
              className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl border-2 border-zinc-200 dark:border-zinc-700 hover:border-[#1F3A8A] dark:hover:border-blue-500 hover:text-[#1F3A8A] dark:hover:text-blue-400 font-medium transition-colors"
            >
              <Download size={16} />
              Download PDF
            </a>
