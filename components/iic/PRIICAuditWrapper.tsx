...
import { declineIICReportWithAnnotations, processIICReportReview, pushIICReportToFaculty } from '@/lib/actions/iic-approvals'
import { CheckCircle2, XCircle, Send, Megaphone, AlertTriangle, ArrowRight, Loader2, Plus, X, FileText } from 'lucide-react'
...
export function PRIICAuditWrapper({ 
  reportId, 
  reportStatus, 
  rejectionFeedback,
  initialAnnotations = [],
  ...
  const isApproved = reportStatus !== 'pending_pr' && reportStatus !== 'rejected_pr' && reportStatus !== 'rejected_faculty'
  ...
  async function handleApprove() {
    setLoading(true)
    const res = await processIICReportReview(reportId, 'pr', 'approve', feedback)
    if (res?.error) {
      alert(res.error)
      setLoading(false)
    } else {
    ...
    return (
      <div className="bg-rose-900 text-white rounded-[3rem] p-12 shadow-2xl space-y-6 border border-rose-700">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-black uppercase text-lg tracking-tighter">IIC Report Declined</h3>
        </div>
    ...
    <div className="bg-black text-white rounded-[3rem] p-10 shadow-2xl space-y-8">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-mono text-xs uppercase tracking-widest text-zinc-400">
          {reportStatus === 'rejected_faculty' ? 'Faculty Verification Failed Review' : 'PR IIC Audit Terminal'}
        </h3>
      </div>

      {reportStatus === 'rejected_faculty' && (
        <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs font-mono">
          <p className="font-bold mb-1">⚠️ REJECTED BACK BY FACULTY ADVISOR</p>
          <p className="italic">"{rejectionFeedback}"</p>
        </div>
      )}
      ...
      {decision === 'decline' && (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <p className="text-[11px] text-zinc-500 italic">
            Add specific comments for sections you want corrected. CC will receive these instructions inline.
          </p>
      ...
      <div className="space-y-3">
        ...
        <textarea
          placeholder={decision === 'decline' ? "Overall feedback for the Club Coordinator..." : "Optional: Internal audit remarks..."}
          value={feedback}
